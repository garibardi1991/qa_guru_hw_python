...
def test_add_fiction_book_to_cart():
    ...
    with allure.step('Добавить товар "Black & White Diamond Heart" в корзину через API.'):
        response_code = add_product_to_cart(product_url='/addproducttocart/catalog/14/1/1', cookie=cookie)
...
def test_add_jeans_to_cart():
    ...
    with allure.step('Добавить товар "Casual Golf Belt" в корзину через API.'):
        response_code = add_product_to_cart(product_url='/addproducttocart/catalog/40/1/1', cookie=cookie)
        assert response_code == 200

    with allure.step('Проверить, что в корзине содержится добавленный товар.'):
        browser.element('.ico-cart .cart-label').click()
        browser.element('.product-name').should(have.exact_text('Casual Golf Belt'))
